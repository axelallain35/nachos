...
//----------------------------------------------------------------------
void Lock::Release() {
  auto previous_interrupt_state =
      g_machine->interrupt->SetStatus(IntStatus::INTERRUPTS_OFF);
  this->free = true;
  if (not this->waiting_queue->IsEmpty()) {
    this->owner = (Thread *)this->waiting_queue->Remove();
    g_scheduler->ReadyToRun(this->owner);
  }
  g_machine->interrupt->SetStatus(previous_interrupt_state);
